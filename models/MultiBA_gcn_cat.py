...
'''
Comparison with GCN method  4.3.1-8    

Ablation Study about  
                        5) Ablation experiments of feature fusion methods  : cat fusion

 multi branches, 3 Attentions(SA,NA,GA), full conection,  
 hid dim:  192    W: 192*64
...
class GlGraphConv(nn.Module):
    ...
    def __init__(self, in_features, out_features, adj, bias=True):
        ...
        self.out_features = int(out_features//3)
        '''已经借鉴了st-gcn中，节点自身 和 相邻节点 之间，具有不同的学习参数 size=2两个W矩阵 '''       
        self.W = nn.Parameter(torch.zeros(size=(3, in_features,self.out_features), dtype=torch.float))
    ...
    def forward(self, input):
        ...
        output1 = torch.matmul(adj * M, h0) 
        output2=torch.matmul(adj * (1 - M), h1)        
        
        concat=torch.cat([output1,output2],dim=2)
  
        
        f_div_C=F.softmax(self.MM,dim=1)#        
        h3=self.pool(h3.transpose(1, 2)).transpose(1, 2)        
        output3 = torch.matmul(f_div_C, h3)   #64 16  128 
        
        concat=torch.cat([concat,output3],dim=2)
...
class ResGCN_model(nn.Module):
    ...
    def forward(self, x):
        ...
        out= x[:, self.grouped_order, :]  #调整节点顺序
        #imput
        out = self.gconv_input(out).transpose(1, 2)
