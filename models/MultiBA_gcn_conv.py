...
Ablation Study about  
                        5) Ablation experiments of feature fusion methods  : conv fusion

 multi branches, 3 Attentions(SA,NA,GA), full conection,  
 hid dim:  128    W: 128*128   Conv 128*3-->128
...
class GlGraphConv(nn.Module):
    ...
    def __init__(self, in_features, out_features, adj, bias=True):
        ...
        self.W = nn.Parameter(torch.zeros(size=(3, in_features, int(out_features)), dtype=torch.float))        
        self.MM = nn.Parameter(torch.zeros(size=(16, 8), dtype=torch.float))
        
        self.WPool=nn.Parameter(torch.zeros(size=(int(out_features*3), out_features), dtype=torch.float))

        nn.init.kaiming_normal_(self.W.data)
        nn.init.kaiming_normal_(self.MM.data)
        
        nn.init.kaiming_normal_(self.WPool.data)
    ...
    def forward(self, input):
        ...
        output=torch.cat((h0,h1,h3),dim=2)
        output=torch.matmul(output,self.WPool)         
