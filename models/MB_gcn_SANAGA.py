...
'''
Ablation Study about  2) Ablation Study about multi-attention mechanism  2-8
                      3) Effect of full connection layer FCL             3-5

all attention and no FCL Full connect layer
'''
...
class GlGraphConv(nn.Module):
    ...
    def forward(self, input):
        ...
        output = torch.matmul(adj * M, h0) + torch.matmul(adj* (1 - M), h1)  #SA NA
  
#        
        f_div_C=F.softmax(self.MM,dim=1)#        
        h3=self.pool(h3.transpose(1, 2)).transpose(1, 2)        
        y1 = torch.matmul(f_div_C, h3)   #64 16  128                        #GA
